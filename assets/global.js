...
document.addEventListener('alpine:init', () => {
  Alpine.data('progressNav', () => ({
    ...
    buildNav() {
      ...
      this.items = Array.from(sections).map((section, index) => {
        const sectionId = section.id || `section-${index}`;
        const title = section.dataset.title || `Section ${index + 1}`;
        const blocks = section.querySelectorAll('.progress-block');
        const children = Array.from(blocks).map((block, i) => ({
          id: block.id || `${sectionId}-block-${i}`,
          title: block.dataset.title || `Step ${i + 1}`
        }));
        return { id: sectionId, title, children };
      });
    },
    ...
    setActiveSection() {
      ...
      let currentSection = '';
      let currentBlock = '';
      let activeChildEl = null;
      let activeParentEl = null;
      ...
      document.querySelectorAll('.progress-nav a').forEach((el) => {
        el.classList.remove('active-main', 'active-child');
      });

      const sectionData = this.items.find(i => i.id === this.activeSection);
      if (sectionData) {
        const parentLink = document.querySelector(`.progress-nav a[data-id="${this.activeSection}"]`);
        if (parentLink) {
          parentLink.classList.add('active-main');
          activeParentEl = parentLink;
        }

        const childData = sectionData.children.find(c => c.id === this.activeBlock);
        if (childData) {
          const childLink = document.querySelector(`.progress-nav a[data-id="${this.activeBlock}"]`);
          if (childLink) {
            childLink.classList.add('active-child');
            activeChildEl = childLink;
          }
        }
      }

      const trackerTarget = activeChildEl || activeParentEl;
      if (trackerTarget) this.moveTracker(trackerTarget);
    },

    moveTracker(el) {
      if (!this.$refs.tracker || !el) return;
      const rect = el.getBoundingClientRect();
      const wrapperRect = el.closest('.nav-wrapper').getBoundingClientRect();
      const offsetTop = rect.top - wrapperRect.top;
      this.$refs.tracker.style.transform = `translateY(${offsetTop}px)`;
      this.$refs.tracker.style.height = `${rect.height}px`;
    },
    ...
    getBlockIndex(blockId) {
      for (let section of this.items) {
        for (let i = 0; i < section.children.length; i++) {
          if (section.children[i].id === blockId) return i;
        }
      }
      return -1;
    }
